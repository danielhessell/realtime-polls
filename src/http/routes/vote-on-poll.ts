  ...
  server.post("/polls/:pollId/votes", async (request, reply) => {
    ...
    if (sessionId) {
      ...
      if (userPreviousVoteOnPoll && userPreviousVoteOnPoll.optionId !== pollOptionId) {
        ...
        const votes = await redis.zincrby(`poll:${pollId}`, -1, userPreviousVoteOnPoll.optionId);
        voting.publish(pollId, { pollOptionId:userPreviousVoteOnPoll.optionId, votes: Number(votes) });
      }
    }
